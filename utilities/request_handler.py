...
class requestHandler:
    ...
    def handle_requests(
        self, url: str, method: str, args: dict = None, headers: dict | None = None, raw_response: bool = False
        ...
        if method == "get":
            if headers:
                response = requests.get(url=url, params=args, verify=True, headers=headers)
            else:
                response = requests.get(url=url, params=args, verify=True)

        elif method == "post":
            if headers:
                response = requests.post(url=url, data=args, verify=True, headers=headers)
            else:
                response = requests.post(url=url, data=args, verify=True)

        elif method == "put":
            response = requests.put(url=url, params=args, verify=True)

        elif method == "delete":
            response = requests.delete(url=url, params=args, verify=True)
        ...
        if raw_response:
            ...
            if status_code == 200:
                if response_headers["Content-Type"] in [
                    "application/json",
                    "charset=utf-8",
