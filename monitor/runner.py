...
class Runner:
    def __init__(self, logger: logging.Logger | None = None) -> None:
        ...
        for signame in {"SIGINT", "SIGTERM"}:
            self.loop.add_signal_handler(
                getattr(signal, signame), functools.partial(self.ask_exit, signame, self.loop, self.logger)
            )
        self.data_aggr_and_sender: dataAggrAndSender | None = None
        self.netliq_to_telegram: netliqToTelgram | None = None
    ...
    async def periodic_hkd_thesis_plotter(self, pwd, seconds: int, starting_date: str) -> None:
        ...
        if not self.data_aggr_and_sender:
            ...
            if self.is_new_week():
                ...
                self.logger.info("Data for today already present, waiting for tomorrow")
        while True:
            if self.is_new_week():
                await self.data_aggr_and_sender.aggr_and_send_data_hkd()
            await asyncio.sleep(seconds)
    ...
    def is_new_day(self, path: str, today_datetime: datetime | None = None) -> tuple[bool, int]:
        try:
        ...
        if not isinstance(last_day_df, str):
            self.logger.error(
                f"Expected a string for date but got {type(last_day_df)} with value {last_day_df}. Skipping this cycle."
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Sending HKD Thesis to Telegram Channel", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    ...
    parser.add_argument(
        "-v", "--verbose", action="count", default=0, help="Be more verbose. Both -v and -q may be used multiple times."
    )
    parser.add_argument("--log-file", dest="log_file", type=str, nargs="?")
    parser.add_argument("--request-type", dest="request_type", type=str, nargs="?", default="HKD")
    parser.add_argument("--starting-date", dest="starting_date", type=str, nargs="?", default="2022-01-01")
