...
class dataFetcher:
    ...
    def fetch_data_and_export_from_hkma(
        self,
        url_base: str,
        url_extension: str,
        fields: list | None = None,
        from_month: str = "2022-01-01",
        ...
        return self.request_handler.handle_requests(url=url, method="get", args=params)["result"]["records"]
    ...
    def fetch_economics_data(self, url: str, to_fetch: str = "GDP") -> pd.DataFrame:
        ...
        if to_fetch == "GDP":
            for sub_dict in result:
                if (
                    sub_dict["GDP_component"] == "GDP"
                    and sub_dict["freq"] == "Q"
                    and sub_dict["svDesc"] == "Year-on-year % change"
                ):
                    res.append(sub_dict)
            ...
            for sub_dict in result:
                if (
                    sub_dict["freq"] == "M"
                    and sub_dict["svDesc"] == "Year-on-year % change"
                    and sub_dict["sv"] == "CC_CM_1920"
                ):
                    res.append(sub_dict)

        else:
            raise NotImplementedError(f"didn't implement handler for {to_fetch}")
