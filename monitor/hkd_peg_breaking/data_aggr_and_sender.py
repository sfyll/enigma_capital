...
class dataAggrAndSender(telegramHandler):
    __LIBOR_CSV_PATH = (
        "/LIBOR USD.csv"  # didn't find free historical source :( , link is : http://iborate.com/usd-libor/
    )
    ...
    def __init__(self, password: str, beginning_date: str, to_telegram=True) -> None:
        ...
        self.hibor_libor_df: pd.DataFrame | None = None
        self.hibor_libor_df_daily: pd.DataFrame | None = None
    ...
    async def plot_and_send_historical_rates(self, plot_spread=False) -> None:
        ...
        if not plot_spread:
            for idx, values in enumerate(maturity_list):
                self.hibor_libor_df.xs(values, level=1, axis=1).plot(
                    ax=a[idx] if len(maturity_list) > 1 else a, title=values + " Rates", sharex=True
                )
        else:
            for idx, values in enumerate(maturity_list):
                # df[values + " USD-HKD spread"] = df.xs("USD", axis=1)[values] - df.xs("HKD", axis=1)[values]
                self.hibor_libor_df[values + " USD-HKD spread as perc. of US Rates"] = (
                    self.hibor_libor_df.xs("USD", axis=1)[values]
                    - self.hibor_libor_df.xs("HKD", axis=1)[values] / self.hibor_libor_df.xs("USD", axis=1)[values]
                )

                # df[values + " USD-HKD spread"].plot(ax=a[idx] if len(maturity_list)>1 else a, title=values + " USD-HKD spread ", sharex=True)
                self.hibor_libor_df[values + " USD-HKD spread as perc. of US Rates"].plot(
                    ax=a[idx] if len(maturity_list) > 1 else a,
                    title=values + " USD-HKD spread as perc. of US Rates",
                    sharex=True,
                )
    ...
    async def plot_and_send_daily_rates(self, plot_spread=False) -> None:
        ...
        if not plot_spread:
            for idx, values in enumerate(maturity_list):
                self.hibor_libor_df_daily.xs(values, level=1, axis=1).plot(
                    ax=a[idx] if len(maturity_list) > 1 else a, title=values + " Rates", sharex=True
                )
        else:
            for idx, values in enumerate(maturity_list):
                self.hibor_libor_df_daily[values + " USD-HKD spread"] = (
                    self.hibor_libor_df_daily.xs("USD", axis=1)[values]
                    - self.hibor_libor_df_daily.xs("HKD", axis=1)[values]
                )
                self.hibor_libor_df_daily[values + " USD-HKD spread as perc. of US Rates"] = (
                    self.hibor_libor_df_daily.xs("USD", axis=1)[values]
                    - self.hibor_libor_df_daily.xs("HKD", axis=1)[values]
                    / self.hibor_libor_df_daily.xs("USD", axis=1)[values]
                )

                self.hibor_libor_df_daily[values + " USD-HKD spread"].plot(
                    ax=a[0], title=values + " USD-HKD spread ", sharex=True
                )
                self.hibor_libor_df_daily[values + " USD-HKD spread as perc. of US Rates"].plot(
                    ax=a[1], title=values + " USD-HKD spread as perc. of US Rates", sharex=True
                )
    ...
    def get_data_historical(self) -> None:
        ...
        hibor_df = pd.DataFrame(
            self.data_fetcher.fetch_data_and_export_from_hkma(url_base, url_extension, from_month=self.beginning_date)
        )
        ...
        merged_df = pd.concat([libor_df.set_index("Date"), hibor_df.set_index("Date")], axis=1, keys=["USD", "HKD"])
    ...
    def get_data_daily_on(self) -> None:
        ...
        hibor_df = pd.DataFrame(
            self.data_fetcher.fetch_data_and_export_from_hkma(
                url_base, url_extension, fields, from_month=self.beginning_date
            )
        )
        libor_df = pd.read_csv(os.path.realpath(os.path.dirname(__file__)) + self.__LIBOR_CSV_PATH)

        hibor_df.rename(columns={"end_of_date": "Date", "hibor_overnight": "ON"}, inplace=True)
    ...
    def get_merged_df_cpi_and_gdp(self, gdp_df: pd.DataFrame, cpi_df: pd.DataFrame) -> pd.DataFrame:
        gdp_df.rename(columns={"GDP_component": "Real GDP", "period": "date"}, inplace=True)

        cpi_df.rename(columns={"freq": "CPI", "period": "date"}, inplace=True)
