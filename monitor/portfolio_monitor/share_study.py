...
class Investor:
    ...
    def update_units(self, date, units):
        ...
        print(
            f"[Investor Update] {self.name} on {date.strftime('%Y-%m-%d')}: Units Change = {units}, Total Units = {cumulative_units}"
        )
...
class Portfolio:
    ...
    def process(self):
        ...
        self.nav_history = pd.concat(
            [self.nav_history, pd.DataFrame({"date": [initial_date], "nav": [initial_nav]})], ignore_index=True
        )
        ...
        for date in all_dates[1:]:
            ...
            self.nav_history = pd.concat(
                [self.nav_history, pd.DataFrame({"date": [date], "nav": [nav]})], ignore_index=True
            )
    ...
    def plot_shares(self):
        ...
        if share_col_primary in self.shares_df.columns:
            ax1.plot(
                self.shares_df["date"],
                self.shares_df[share_col_primary],
                color="blue",
                label=f"{primary_investor.capitalize()}'s Share",
            )
            ax1.set_xlabel("Date")
        ...
        for investor in investor_names:
            if investor != primary_investor:
                ...
                if share_col in self.shares_df.columns:
                    ax2.plot(
                        self.shares_df["date"],
                        self.shares_df[share_col],
                        label=f"{investor.capitalize()}'s Share",
    ...
    def plot_shares_with_inset(self):
        ...
        for investor in self.investors.keys():
            if investor != primary_investor:
                ...
                if share_col in self.shares_df.columns:
                    ax_inset.plot(
                        self.shares_df["date"], self.shares_df[share_col], label=f"{investor.capitalize()}'s Share"
                    )
    ...
    def update_to_latest_netliq(self):
        ...
        latest_nav = (
            self.netliq_data[self.netliq_data["date"] == latest_netliq_date]["netliq"].iloc[0] / self.total_units
        )
