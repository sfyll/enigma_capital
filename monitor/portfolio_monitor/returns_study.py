...
class returnStudy:
    ...
    def get_netliq_path(self) -> str:
        ...
        return (
            base,
            base + "/account_data_fetcher/csv_db/balance.csv",
    ...
    def construct_twr(self, start_date: str | None = None, end_date: str | None = None, exchange: str | None = None):
        ...
        if exchange is not None:
            ...
            merged_data["daily_return"] = (
                merged_data["netliq"] / (merged_data["netliq"].shift(1) + merged_data["net_amount"])
            ) - 1
    ...
    def get_transaction_data_per_exchange(self, exchange, transactions_data) -> pd.DataFrame:
        transactions_data = transactions_data[
            (transactions_data["from_exchange"] == exchange.upper())
            | (transactions_data["to_exchange"] == exchange.upper())
        ]
    ...
    def get_transaction_data_portfolio(self, transactions_data) -> pd.DataFrame:
        ...
        transactions_data = transactions_data[
            (transactions_data["from_exchange"] == 0) | (transactions_data["to_exchange"] == 0)
        ]
