...
class dydxDataFetcher(accountFetcherBase):
    _EXCHANGE = "DYDX"
    def __init__(self, path: str, password: str) -> None:
        ...
        self.client = Client(
            ...
            api_key_credentials={
                    'secret': self.api_meta_data[self._EXCHANGE].secret,
                    'key': self.api_meta_data[self._EXCHANGE].key,
                    'passphrase': self.api_meta_data[self._EXCHANGE].other_fields["Passphrase"],
                }
        )
