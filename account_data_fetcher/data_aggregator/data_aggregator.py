...
class AggregatedData:
    ...
    def get_object_if_ready(self) -> dict | None:
        """
    ...
    def __are_all_reports_new_day(self) -> bool:
        ...
        last_sent_date: date | None = datetime.strptime(self.date, "%Y-%m-%d %H:%M:%S").date() if self.date else None
...
class DataAggregator:
    ...
    def __init__(
        self,
        aggregation_interval: int,
        fetcher_instances: dict[str, ExchangeBase],
            ...
            while True:
                ...
                for name, data in named_results.items():
                    try:
                        ...
                        if data is None:
                            self.logger.info(
                                f"No data returned for {name}, but no exception was raised. Skipping update."
                            )
                            continue
                ...
                if objects_to_send:
                    self.logger.info(
                        f"Data is valid and ready. Pushing aggregated object to {len(self.output_queues)} writers."
                    )
