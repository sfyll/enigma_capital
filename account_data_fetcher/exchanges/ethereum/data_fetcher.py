...
class DataFetcher(ExchangeBase):
    ...
    async def get_prices_for_coins_async(self, balance_by_coin: dict[str, float]) -> None:
        ...
        coins_to_fetch = [
            coin for coin, balance in balance_by_coin.items() if "USD" not in coin.upper() and balance > 0
        ]
    ...
    def __query_multi_call_sync(self) -> list[bytes]:
        ...
        for coin, token_address in self.__ADDRESS_BY_COIN.items():
            for address in self.address_of_interest:
                balance_call_data = self.contract_by_coin[coin].encode_abi(
                    "balanceOf", args=[Web3.to_checksum_address(address)]
                )
                calls.append((Web3.to_checksum_address(token_address), balance_call_data))

        multicall_contract = self.w3.eth.contract(
            address=Web3.to_checksum_address(MULTICALL_3_ADDRESS), abi=MULTICALL3_ABI
        )
    ...
    def encode_token_balances_by_coin_calls(self):
        ...
        for coin, contract in self.contract_by_coin.items():
            for my_address in [
                "0x9527465642a7015738ef24201eec1644f3755670",
                "0x0f366a411dc9f8a1611cad614d8f451436fc4f4b",
                "0x630276c20064545c06360bbd3ef48025abe3316a",
                "0xbe6e784ad98581be1077ddf630205ac30ce8128b",
