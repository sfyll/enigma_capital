...
class DataFetcher:
    ...
    def __init__(self, session: aiohttp.ClientSession):
        ...
        self.id_per_symbol: dict[str, str] | None = None
        self.symbol_per_id: dict[str, str] | None = None
    ...
    async def get_prices(self, symbols: list[str], vs_currencies: list[str] = ["USD"]) -> dict[str, dict[str, float]]:
        ...
        ids_to_fetch = [
            self.id_per_symbol[symbol.upper()] for symbol in symbols if symbol.upper() in self.id_per_symbol
        ]
        if not ids_to_fetch:
