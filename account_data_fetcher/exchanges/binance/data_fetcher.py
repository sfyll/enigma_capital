...
class DataFetcher(ExchangeBase):
    ...
    def get_spot_positions(self) -> dict:
        ...
        assets_to_price = {
            ua["asset"] for ua in user_assets if float(ua["btcValuation"]) > 0.01 and ua["asset"] not in self.STABLES
        }
        price_map = self._batch_usdt_prices(assets_to_price)
    ...
    def save_historical_klines(
        self, symbol, file_loc: str, start: str = "2020/01/01", end: str = "2023/01/01", interval="1m"
    ) -> None:
        ...
        for cross in saved_prices:
            if symbol.upper() + ".csv" == cross:
                ...
                end_ts_old = (
                    int(
                        time.mktime(
                            datetime.datetime.strptime(df["Unnamed: 0"].iloc[-1], "%Y-%m-%d %H:%M:%S.%f").timetuple()
                        )
                    )
                    * 1000
                )
    ...
    def get_deposit_history(self, path: str, start_date: str = "01/01/2020", end_date: str | None = None) -> None:
        start_timestamp = int(time.mktime(datetime.datetime.strptime(start_date, "%m/%d/%Y").timetuple()) * 1000)
        end_timestamp = (
            int(time.time() * 1000)
            if end_date is None
            else int(time.mktime(datetime.datetime.strptime(end_date, "%m/%d/%Y").timetuple()) * 1000)
        )
        ...
        while current_start_timestamp < end_timestamp:
            current_deposits = self._signed_call(
                self._client.deposit_history, startTime=current_start_timestamp, endTime=current_end_timestamp
            )
        ...
        df.drop(
            columns={"id", "addressTag", "transferType", "confirmTimes", "unlockConfirm", "walletType"},
            errors="ignore",
            inplace=True,
        )
        df.to_csv(path)

    def get_withdraw_history(self, path: str, start_date: str = "01/01/2020", end_date: str | None = None) -> None:
        start_timestamp = int(time.mktime(datetime.datetime.strptime(start_date, "%m/%d/%Y").timetuple()) * 1000)
        end_timestamp = (
            int(time.time() * 1000)
            if end_date is None
        ...
        withdrawals = []
        while current_start_timestamp < end_timestamp:
            current_withdrawals = self._signed_call(
                self._client.withdraw_history, startTime=current_start_timestamp, endTime=current_end_timestamp
            )
        ...
        df.drop(
            columns={"id", "applyTime", "transferType", "info", "confirmNo", "walletType", "txKey"},
            errors="ignore",
            inplace=True,
        )
        df.to_csv(path)
