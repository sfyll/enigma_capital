...
class DataFetcher(ExchangeBase):
    ...
    def __get_balances(self, accountType="UNIFIED") -> float:
        if accountType == "UNIFIED":
            ...
            try:
                derivative_balance = float(
                    self.bybit_connector.get_derivative_balance(accountType=accountType)[0]["totalEquity"]
                )
            except InvalidRequestError as e:
                raise e
    ...
    def __get_spot_positions(self) -> dict:
        ...
        for position in positions:
            ...
            dollar_quantity = (
                quantity
                if position["coin"].upper() in ["BUSD", "USDC", "USDT"]
                else quantity * self.__get_coin_price(position["coin"])
            )
    ...
    def __get_unified_positions(self) -> dict:
        ...
        for position in positions:
            ...
            if quantity > 0:
                dollar_quantity = (
                    0
                    if position["coin"].upper() in ["BUSD", "USDC", "USDT", "DAI"]
                    else quantity * self.__get_coin_price(position["coin"])
                )
            if dollar_quantity > 100:
                data_to_return["Symbol"].append(position["coin"])
    ...
    def __get_derivatives_positions(self) -> dict:
        ...
        for position in positions["list"]:
            if position["size"]:
                ...
                data_to_return["Dollar Quantity"].append(
                    round(
                        float(position["positionValue"])
                        + float(position["cumRealisedPnl"])
    ...
    def __get_coin_price(self, symbol: str) -> float:
        ...
        try:
            return float(
                self.bybit_connector.get_last_traded_price(category="spot", symbol=symbol + "USDT")["lastPrice"]
            )
        ...
        try:
            return float(
                self.bybit_connector.get_last_traded_price(category="spot", symbol=symbol + "USD")["lastPrice"]
            )
        except InvalidRequestError:
            pass
        ...
        try:
            return float(
                self.bybit_connector.get_last_traded_price(category="spot", symbol=symbol + "USDC")["lastPrice"]
            )
        except InvalidRequestError:
            pass
